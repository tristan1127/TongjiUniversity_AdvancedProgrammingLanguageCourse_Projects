...
void cct_setcursor(const int options)
{
	...
	switch (options) {
		case CCT_CURSOR_VISIBLE_FULL:
			cursor_info.bVisible = 1;
			cursor_info.dwSize = 100;
			break;
		case CCT_CURSOR_VISIBLE_HALF:
			cursor_info.bVisible = 1;
			cursor_info.dwSize = 50;
			break;
		case CCT_CURSOR_INVISIBLE:
			cursor_info.bVisible = 0;
			cursor_info.dwSize = 1;
			break;
		case CCT_CURSOR_VISIBLE_NORMAL:
		default: //缺省显示光标，横线
			cursor_info.bVisible = 1;
...
void cct_showstr(const int X, const int Y, const char *str, const int bg_color, const int fg_color, int rpt, int maxlen)
{
	...
	bool is_first_half = false; //是否是汉字的第一个字节，默认不是
	...
	for (i = 0, p = str; i < maxlen; i++, p++) {	//重复rpt次，每次输出字符串，适用于在画边框时输出若干个"═"等情况
		if (*p == 0) {
			...
			rpt_count++;
			is_first_half = false; //对于rpt>1的重复情况，恢复false
		}
		...
		if ((*p != '\0') && (*(p+1) != '\0')) {
			/* p是否指向了汉字的前半个 */
			if ((unsigned char)(*p) >= 0xA1 && (unsigned char)(*p) <= 0xFE) //目前仅限于GB2312的判断
				is_first_half = !is_first_half; //汉字则在false/true之间来回变化
			else
				is_first_half = false;

			/* 如果未到字符串尾部 && 不是最后一个字符 && 是汉字的前半个，则判断是否边框线 */
			bool found = false;
			for (int sp_no = 0; is_first_half && special[sp_no] != NULL; sp_no++) {
				if (strncmp(p, special[sp_no], strlen(special[sp_no])) == 0) {
					found = true;
					break;
				}
			}
			/* 如果是是边框的两个字符，则输出三个字符（边框的两个字符+空格） */
			if (found) {
				putchar(*p);
				++i; //多加一次
				++p; //多加一次
				putchar(*p);
				putchar(' '); //多补一个空格
				is_first_half = false; //已经处理完一个边框汉字，恢复false
			}
			else { //不是边框，原样输出
				putchar(*p);
			}
...
//	PROCGETCONSOLEWINDOW GetConsoleWindow = (PROCGETCONSOLEWINDOW)GetProcAddress(hKernel32,"GetConsoleWindow");
	...
	GetCurrentConsoleFontEx(__hout, 0, &infoex);
...
/***************************************************************************
  ...
  输入参数：
  返 回 值：
  说    明：1、新版控制台的可选字体，可以在打开cmd窗口后，鼠标单击左上角菜单 - “属性” - “字体卡片”中查看
            2、由调用本函数的使用者保证给出的fontname是正确的，如果错误，则不保证显示效果
			 "MS Gothic"、"SimSun-ExtB"、"SimSun-ExtG"、"点阵字体"、"仿宋"、"黑体"、"楷体"、"隶书"、"新宋体"、"幼圆"
            3、如果既要设置窗口大小，又要设置字体，则必须先设置字体大小，再设置窗口大小
			（使用者要保证字体+窗口大小设置后，不超过你当前显示器设置的分辨率的可显示范围）
***************************************************************************/
